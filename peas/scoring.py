...
def compute_mean_table_2d(data, start_diagonal=1, end_diagonal=0):
    ...
    if end_diagonal - start_diagonal >= 2:
        ...
        mean_table[d2_idx] = data[d2_idx] + mean_table[truncate_array_tuple(d1_idx, 1, 0)] + mean_table[
        ...
        if end_diagonal - start_diagonal >= 3:
            ...
            for diagonal_idx in range(start_diagonal + 2, end_diagonal):
                ...
                mean_table[dk_idx] = data[dk_idx] + mean_table[truncate_array_tuple(dk_prev, 1, 0)] + mean_table[
                    truncate_array_tuple(dk_prev, 0, 1)] - mean_table[truncate_array_tuple(dk_prevprev, 1, 1)]
...
def compute_mean_table_1d(data_vec, start_diagonal=0, end_diagonal=0):
    ...
    mean_table[d0_idx] = data_vec

    if n > 1:
        # Second diagonal is left and beneath cells on 0th diagonal
        d1_idx = my_diag_indices(n, 1)
        #         print(d1_idx)
        mean_table[d1_idx] = mean_table[truncate_array_tuple(d0_idx, 1, 0)] + mean_table[
        ...
        if n > 2:
            ...
            for k in range(2, n):
                ...
                mean_table[dk_idx] = mean_table[truncate_array_tuple(dk_prev, 1, 0)] + mean_table[
                    truncate_array_tuple(dk_prev, 0, 1)] - mean_table[truncate_array_tuple(dk_prevprev, 1, 1)]
