...
def flatten_triu(triu_matrix, start_diagonal, end_diagonal):
    """
    Returns a 1D-vector generated by concatenating all the diagonals of 
    :param:`triu_matrix` from :param:`start_diagonal` 
    to :param:`end_diagonal`
    """
    n = get_square_matrix_size(triu_matrix)
    d = end_diagonal - start_diagonal + 1

    flat_array = numpy.empty(int(d * (n - (start_diagonal + end_diagonal)/2)), dtype=triu_matrix.dtype)
    
    flat_array_idx = 0
    for diagonal in range(start_diagonal, end_diagonal + 1):
        diagonal_length = n - diagonal
        flat_array[flat_array_idx:flat_array_idx+diagonal_length] = numpy.diag(triu_matrix, diagonal)
        flat_array_idx += diagonal_length
        
    return flat_array

    
def ravel_triu(arr, start_diagonal, end_diagonal, empty_fill_value=0):
    """
    Returns an upper-triangular matrix whose diagonals from :param:`start_diagonal` 
    to :param:`end_diagonal` are the consecutive contents  of param:`arr`.
    """
    l = len(arr)
    n = int((2 * l - start_diagonal**2 +start_diagonal + end_diagonal**2 + end_diagonal) / (2 * (end_diagonal - start_diagonal +1)))
    triu_matrix = numpy.full((n,n), fill_value=empty_fill_value, dtype=arr.dtype)
